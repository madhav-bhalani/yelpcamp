...
app.get('/createCampground', async(req,res)=>{
    try{
        const camp = new Campground({title: 'My Backyard', description: 'cheap camping', price: 12.44});
        await camp.save();
        res.send(camp);
    }  
    catch(err){
...
app.put('/campgrounds/:id', async(req,res)=>{
    try{
        ...
        const campground = await Campground.findByIdAndUpdate(id, {title: newTitle, location: newLoc});
